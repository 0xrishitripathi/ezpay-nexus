      ...
      <div className="fixed inset-0 z-50 overflow-y-auto">
        ...
        <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto relative">
          ...
          <div className="p-6 space-y-6">
            {/* Title */}
            <div>
              <h3 className="text-2xl font-bold text-gray-900">{transaction.title}</h3>
            </div>
            ...
            <div className="bg-gray-50 rounded-lg p-4 space-y-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-medium text-gray-900">{totalPeople} {totalPeople === 1 ? 'person' : 'people'}</span>
              </div>

              <div className="pt-3 border-t border-gray-200">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-gray-900">Your Share</span>
                  <span className="text-xl font-bold text-blue-600">${transaction.perPersonAmount || transaction.amount}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between p-4 bg-blue-50 rounded-lg">
              ...
              <span className="text-sm font-semibold text-gray-900">{transaction.paidByName || 'You'}</span>
            </div>
            ...
            {transaction.participants && transaction.participants.length > 0 && (
              <div>
                <div className="flex items-center mb-3">
                  ...
                  <h4 className="text-sm font-semibold text-gray-900">Split With</h4>
                </div>
                <div className="space-y-2">
                  ...
                  <div className="flex items-center justify-between p-3 bg-blue-50 rounded-lg border border-blue-200">
                    ...
                    <span className="text-sm font-semibold text-blue-600">
                      ${transaction.perPersonAmount || (parseFloat(transaction.amount) / totalPeople).toFixed(2)}
                    </span>
                  </div>
                  ...
                  {transaction.participants.map((participant) => (
                    ...
                    >
                      <div className="flex items-center">
                        <div className={`w-10 h-10 rounded-full bg-gradient-to-br ${getAvatarGradient(participant.name)} flex items-center justify-center text-white font-semibold mr-3`}>
                          {participant.name.charAt(0).toUpperCase()}
                        </div>
                        <div>
                          <span className="text-sm font-medium text-gray-900">{participant.name}</span>
                          <p className="text-xs text-orange-600 font-medium">Due</p>
                        </div>
                      </div>
