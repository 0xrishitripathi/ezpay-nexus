...
const TransactionItem = ({ 
  ...
  const getIcon = () => {
    ...
    } else if (lowerTitle.includes('cafe') || lowerTitle.includes('restaurant')) {
      return <Utensils className="w-5 h-5 text-orange-500" />;
    } else if (lowerTitle.includes('coffee') || lowerTitle.includes('starbucks')) {
  ...
  return (
    <div 
      className={`flex items-center justify-between py-4 px-6 ${onClick ? 'cursor-pointer hover:bg-gray-50 transition-colors' : ''}`}
      onClick={onClick}
    >
                ...
                <div className="flex -space-x-2">
                  {participants.slice(0, 3).map((participant, index) => (
                    <div
                      ...
                      className={`w-6 h-6 rounded-full bg-gradient-to-br ${getAvatarGradient(participant.name)} flex items-center justify-center text-white text-xs font-semibold border-2 border-white`}
